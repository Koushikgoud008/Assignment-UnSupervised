...
st.set_page_config(page_title="PCA Dimensionality Reduction", layout="wide", initial_sidebar_state="collapsed")
...
st.markdown("<h1 style='text-align: center; font-weight: normal; font-size: 3rem; margin-bottom: 0;'>Retail Data<em class='accent'>PCA Mapping</em></h1>", unsafe_allow_html=True)
st.markdown("<p class='subtext' style='text-align: center; margin-top: 10px;'>Compress multi-dimensional retail metrics into a 2D principal component space.</p>", unsafe_allow_html=True)
...
with col1:
    with st.container(border=True):
        ...
        with st.form("pca_form"):
            ...
            predict_btn = st.form_submit_button("RUN PCA TRANSFORMATION", use_container_width=True)
...
with col2:
    with st.container(border=True):
        if predict_btn:
            ...
            components, explained_variance = predict_new_data(input_df, scaler)
            pc1 = components[0][0]
            pc2 = components[0][1]
            ...
            st.markdown(f"<h3 style='text-align: center; margin-bottom: 5px; transition: all 0.3s ease;'>Projected Space</h3>", unsafe_allow_html=True)
            
            st.markdown(f"""
            <div style='display: flex; justify-content: center; gap: 40px; margin-top: 20px;'>
                <div style='text-align: center;'>
                    <p class='subtext' style='margin-bottom: 0;'>Principal Component 1</p>
                    <h2 class='accent' style='margin-top: 5px;'>{pc1:.4f}</h2>
                </div>
                <div style='text-align: center;'>
                    <p class='subtext' style='margin-bottom: 0;'>Principal Component 2</p>
                    <h2 class='accent' style='margin-top: 5px;'>{pc2:.4f}</h2>
                </div>
            </div>
            """, unsafe_allow_html=True)
            
            st.markdown(f"<p class='microtext' style='text-align: center; margin-top: 25px;'>These two components capture <b>{total_variance_explained:.1f}%</b> of the original data's variance.</p>", unsafe_allow_html=True)
            st.markdown("<br><br>", unsafe_allow_html=True)
        else:
            st.markdown("<br><br><br>", unsafe_allow_html=True)
            st.markdown("<h1 class='microtext' style='text-align: center; font-size: 3rem;'>✦</h1>", unsafe_allow_html=True)
            st.markdown("<h4 class='subtext' style='text-align: center; margin-bottom: 5px;'>Awaiting PCA Pipeline</h4>", unsafe_allow_html=True)
            st.markdown("<p class='microtext' style='text-align: center;'>Submit the raw transaction vectors on the left to project<br>them into the compressed dimensionality space.</p>", unsafe_allow_html=True)
            st.markdown("<br><br><br>", unsafe_allow_html=True)